...
const makeSet = () => ({ id: crypto.randomUUID(), weight: '', reps: '', rpe: '', completed: false })

export default function ExerciseCard({ exercise, onSetsChange }) {
  const [sets, setSets] = useState(() => Array.from({ length: exercise.sets || exercise.targetSets || 0 }, () => makeSet()))
  ...
  const updateSet = (id, next) => { const updated = sets.map((set) => (set.id === id ? next : set)); setSets(updated); onSetsChange?.(updated) }
  const removeSet = (id) => { const updated = sets.filter((set) => set.id !== id); setSets(updated); onSetsChange?.(updated) }
    ...
    <article className="panel p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="muted mt-1">Target {exercise.sets || exercise.targetSets} sets · {exercise.targetReps} reps · {exercise.rest || exercise.restTime}s rest</p>
        </div>
      ...
      <div className="mt-4 rounded-lg bg-slate-50 p-3 dark:bg-neutral-950">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          <span className="text-sm text-slate-500">No previous session</span>
        </div>
      </div>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        <Button variant="secondary" size="sm" onClick={() => { const updated = [...sets, makeSet()]; setSets(updated); onSetsChange?.(updated) }}><Plus size={15} />Add Set</Button>
